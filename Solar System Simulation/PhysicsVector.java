...
public class PhysicsVector {
    ...
    **/
    public static PhysicsVector subtract(PhysicsVector v, PhysicsVector u){
        ...
        /**
    * vectorProduct is a standard vector product. If <b> v = xi + yj + zk</b>
    * and <b>u = mi + ni + pk</b>, then the vector product returns <b>(y*p-n*z)i
    * + (z*m-x*p)j + (x*n-y*m)k</b>
    *
    * @param v first vector in product
    * @param u second vector in product
    * @return return scalar product
    **/
    public static PhysicsVector vectorProduct(PhysicsVector v, PhysicsVector u){
		PhysicsVector vectorProduct = new PhysicsVector();
		for (int i=0; i<v.vectorComponents.length; i++) 
				vectorProduct.vectorComponents[i]=v.vectorComponents[(i+1)%3]*u.vectorComponents[(i+2)%3]-v.vectorComponents[(i+2)%3]*u.vectorComponents[(i+1)%3];
